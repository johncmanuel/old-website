...
const TextAnim = dynamic(
    () => import('components/writing_anim'),
    { ssr: false }
)
...
export default function Writing() {
    return <Layout>
        ...
        <section className="container-fluid main-section nopad">
            <div className="center-page center-img" id={styles["writing-landing-img"]}>
                    ...
                    {NOVELS.map((item, key) => (
                        <div className="col mb-4" key={key}>
                            <div className="card bg-light add-shadow">
                                <img src={item.cover} className="card-img-top img-fluid" alt={item.alt} />
                                <div className="card-body">
                                    <h5 className="card-title">{item.name}</h5>
                                    <Link href={`/writing/works/[id]`} as={`/writing/works/${item.name}`}>
                                        <a className="btn card-link">More Info</a>
                                    </Link>
                                </div>
                            </div>
                        </div>
