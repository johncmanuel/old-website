...
export default function Home() {
  ...
  return <Layout>
    ...
    <section className="container-fluid main-section nopad">
      <div className="center-page center-img" id="homepage-landing-img">
        ...
        <div className="page-scroll-section">
          ...
          <a data-scroll href="#aboutme" className="page-scroll" id="scroll-anim">
            <svg xmlns="http://www.w3.org/2000/svg" x="0px" y="0px" width={52} height={52} viewBox="0 0 172 172" style={{ fill: '#000000' }}><g fill="none" fillRule="nonzero" stroke="none" strokeWidth={1} strokeLinecap="butt" strokeLinejoin="miter" strokeMiterlimit={10} strokeDasharray="true" strokeDashoffset={0} fontFamily="none" fontWeight="none" fontSize="none" textAnchor="none" style={{ mixBlendMode: 'normal' }}><path d="M0,172v-172h172v172z" fill="none" /><g fill="#ffffff"><path d="M154.74625,44.6125c-1.8275,0.05375 -3.5475,0.80625 -4.81062,2.12313l-63.93563,63.93562l-63.93562,-63.93562c-1.30344,-1.33031 -3.07719,-2.08281 -4.945,-2.08281c-2.795,0 -5.30781,1.70656 -6.35594,4.3c-1.06156,2.59344 -0.43,5.56312 1.57219,7.51156l68.8,68.8c2.6875,2.6875 7.04125,2.6875 9.72875,0l68.8,-68.8c2.0425,-1.96188 2.6875,-4.98531 1.58562,-7.60563c-1.08844,-2.62031 -3.66844,-4.3 -6.50375,-4.24625z" /></g></g></svg>
          </a>
        </div>
      </div>
    </section>
    ...
    <section className="container-fluid main-section" id="aboutme">
      <div className="container">
        <h1 className="section-title text-center">ABOUT ME</h1>
          <h5 className="pad20 text-center" style={{ fontSize: '1.3em', letterSpacing: '1px', lineHeight: '1.5em' }}>
            For the longest time,
            I was fascinated by technology and storywriting. I wrote my first chapter at nine years old and
            executed my first line of code at 10. By utilizing both algorithmic thinking and vivid creativity,
            I make projects that bring meaning to not only myself, but for others too.
          </h5>
        ...
        <div className="card-group">
          ...
          {TABLE.map((item, table_key) => (
            <div className="card add-shadow text-center bg-light" key={table_key}>
              <div className="card-body">

                {/* Display appropiate SVG */}
                {showSvg(svgs, item.svg_id)}
                <h1 className="card-title padtop20">{item.title}</h1>
                
                <br />

                {/* Iterate through objects in "category" property */}
                {item.category.map((category, category_key) => (
                  <>
                    <h4 className="card-text" key={category_key}>{category.name}</h4>

                    {/* Iterate through "values" property inside "category" */}
                    {category.values.map((values, values_key) => (
                      <h6 className="card-text" key={values_key}>{values}</h6>
                    ))}
        ...
        <h6 className="padtop20">*See my Software page for more details.</h6>
      </div>
    </section>
    {/* Navigation Buttons */}
          ...
          <div className="row">
            {items.map((item, key) => (
              <div className="col pad20-top-bottom" key={key}>
                <h1 className="section-title black">{item.item.toUpperCase()}</h1>
                  <h3 className="pad20 section-caption">{item.caption}</h3>
                  <Link href={item.link}>
                    <a className="btn btn-outline-dark btn-lg font15" role="button">{item.item}</a>
                  </Link>
                </div>
            ))}
            </div>
